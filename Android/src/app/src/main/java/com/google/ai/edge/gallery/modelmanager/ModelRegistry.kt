...
package com.google.ai.edge.gallery.modelmanager
...
// edits heavily; this fork re-merges it. Relocating those helpers out of Google's class -- or
...
// Deliberately merged-base's ordering, NOT the fork's reordered variant -- the reorder is a
...
constructor(
   ...
   * added after the fork, so it stays defined here rather than being ported anywhere.
   */
  ...
  fun cleanupModel(
    ...
    if (com.google.ai.edge.gallery.openai.OpenAiServerState.isPinned(model.name)) {
      onDone()
      return
    }
